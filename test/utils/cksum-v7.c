    ...
    char **argv;
{
    ...
    do {
        ...
        if (ferror(fp)) {
            ...
            tmplen = len;
            while (tmplen > 0) {
                crc = crcadd(crc, ((unsigned)tmplen) & 0xFF);
                tmplen = (tmplen >> 8) & 0xFFFFFFL;
            }
            crc = crcfinal(crc);
            printf("%s ", u32toa(crc));
            printf("%s", u32toa(len));
...
/* Adapted from b_dconv() in 2.9BSD source tree. */
char *
u32toa(value)
    long value;
{
