...
on modern operating systems. The current version of *retro-fuse* supports mounting
filesystems created by Fourth-, Fifth-, Sixth-, and Seventh-Edition Research Unix,
as well as 2.9BSD and 2.11BSD. It can also initialize such filesystems.

The design of *retro-fuse* is novel in that it incorporates original source code
from historic Unix kernels. The ancient code has been "lightly modernized" to be
...
to support both reading and writing ancient filesystems, while ensuring the historical
...
system and checking filesystem's integrity and contents (using legacy tools such as icheck
and fsck).
...
**Version 5 (2021-11-10)** — Various code restructuring and cleanup. Fixed a
...
**Version 2 (2021-03-02)** — Fixed a bug in link(), unlink(), chmod(), and chown()
that caused file modification times to be updated (this fixes problems
